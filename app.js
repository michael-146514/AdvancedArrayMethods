...
function problemOne(dishes, cuisine) {
  let results;
  results = dishes.filter(function (el) {
    if (el === cuisine) {
      return true;
...
console.log(problemOne(dishes, "Vegetarian"));

//2. Create a function that will prompt the user to enter a cuisine type and then return all dishes that match that type
//Filter

function problemTwo(dishes, cuisine) {
  let results;
  results = dishes.filter(function (el) {
    if (el === cuisine) {
      return true;
...
let selectedCuisine = prompt("What cuisine would you like?");

console.log(problemTwo(dishes, selectedCuisine));

//3. Create a function that will return all dishes with the cuisine type of "Italian" and a serving size greater than 5.
//Filter

function problemThree(dishes, cuisine) {
  let results;
  results = dishes.filter(function (el) {
    if (el === cuisine && dishes.servings > 5) {
      return true;
    } else {
      return false;
    }
...
console.log(problemThree(dishes, "Italian"));
...
function problemSeven(selectedIngredient) {
  let results;
  results = dishes.filter(function (el) {
    if (el.ingredients.includes(selectedIngredient)) {
      return true;
    } else {
      return false;
    }
...
selectedIngredient = prompt("Select an Ingredient");
console.log(problemSeven(selectedIngredient));
...
console.log(problemNine(dishes));

//10. Create a function that will use advanced array methods on the 'dishes' array and return the result ["Vegetarian Lasagna", "Vegetarian Falafel", "Vegetarian Chili"]

function problemTen(dishes) {
  let vegetarianDishes = dishes
    .filter(function (dish) {
      return dish.name.includes("Vegetarian");
    })
    .map(function (dish) {
      return dish.name;
    });
...
console.log(vegetarianDishes(dishes));
